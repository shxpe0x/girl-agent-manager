...
#   2. Ставит сам пакет `@shxpe0x/manager-agent` в тот же изолированный prefix.
...
flags:
  ...
  --version=X.Y.Z     версия @shxpe0x/manager-agent (по умолч. latest)
...
install_local() {
  ...
  say "ставлю @shxpe0x/manager-agent@${PKG_VERSION} в локальный prefix..."
  mkdir -p "$PREFIX/lib"
  # `npm install --prefix <dir>` — изолированная установка, не трогает глобал
  "$NODE" "$NPM" install --prefix "$PREFIX/lib" --no-audit --no-fund --silent "@shxpe0x/manager-agent@${PKG_VERSION}" \
    || die "npm install не удался"

  cat >"$BIN_DIR/manager-agent" <<EOF
#!/usr/bin/env sh
# manager-agent local node shim — generated by install.sh
exec "${PREFIX}/runtime/bin/node" "${PREFIX}/lib/node_modules/@shxpe0x/manager-agent/dist/cli.js" "\$@"
EOF
  chmod +x "$BIN_DIR/manager-agent"
...
install_termux() {
  ...
  say "ставлю @shxpe0x/manager-agent@${PKG_VERSION} глобально (в Termux $PREFIX)"
  npm install -g --no-audit --no-fund --omit=optional --ignore-scripts "@shxpe0x/manager-agent@${PKG_VERSION}" >&2 \
...
обновить: npm install -g @shxpe0x/manager-agent@latest
удалить: npm uninstall -g @shxpe0x/manager-agent
